...
export default function CitizenProfile() {
  ...
  const saveProfile = async (e) => {
    ...
    try {
      await updateUser(user._id, { name: profile.name.trim() });
      await refreshUser();
  ...
  const savePassword = async (e) => {
    ...
    setSavingPw(true);
    try {
      await changePassword(user._id, { oldPassword: pw.oldPassword, newPassword: pw.newPassword });
        ...
        <SectionCard title="Account">
          ...
          <dl className="profile-dl">
            ...
            <div className="detail-row">
              ...
              <span className="detail-value">{user?.rewardPoints ?? 0} pts</span>
            </div>
          </dl>
        </SectionCard>
          ...
          <SectionCard title="Change password">
            <form onSubmit={savePassword}>
              <div className="form-group">
                ...
                <input className="form-input" type="password" value={pw.oldPassword} onChange={(e) => setPw((p) => ({ ...p, oldPassword: e.target.value }))} placeholder="Current password" />
              </div>
              <div className="form-group">
                <label className="form-label">New password</label>
                <input className="form-input" type="password" value={pw.newPassword} onChange={(e) => setPw((p) => ({ ...p, newPassword: e.target.value }))} placeholder="Min 6 characters" />
              </div>
              <div className="form-group">
